...
class RouteServerConnection {
    ...
    private fun getData(methodName: String, requestMethod:String,  postParam: JSONObject?, errorArrayList: ArrayList<ErrorMessage> ): JSONArray? {
        ...
        try {
            ...
            errorArrayList.add(ErrorMessage("Ошибка загрузки данных", "Ошибка соединения с сервером",e))
        }
        ...
        return try {
            ...
            if (code == 200) {
                try {
                    ...
                    errorArrayList.add(ErrorMessage("Ошибка загрузки данных", "Ошибка чтения потока/формата данных",e))
                    null
                ...
                errorArrayList.add(ErrorMessage("Ошибка загрузки данных",  connector.responseMessage,null))
        ...
        if (data!=null) {
            for (i in 0 until data.length()) {
                try {
                    ...
                    errorArrayList.add(ErrorMessage("Ошибка загрузки данных", "Ошибка создания элемента класса", e))
                }
            }
        }
